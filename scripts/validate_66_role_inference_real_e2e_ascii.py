...
def main() -> None:
    print("=" * 92)
    print("VALIDACAO 66 - END-TO-END REAL: PROVIDER -> CACHE -> TRANSFORMER -> ROLE INFERENCE")
    print("=" * 92)
    ...
    if MATCH_INDEX_PATH.exists():
        ...
        index = {}
        print(
            "[AVISO] match_index.json n?o encontrado; "
            "o teste tentar? obter um PUUID do pr?prio payload"
        )

    # Provider real da aplica??o.
    ...
    check(
        manual_count == 139,
        "provider real expoe os 139 itens manuais V2",
    )
    ...
    check(
        bool(unit_catalog),
        "catalogo manual de unidades Set 18 carregado",
    )
    ...
    for path in cache_files:
        ...
        if not isinstance(payload, dict):
            failed_transform += 1
            transform_errors.append(
                f"{path.name}: payload nao e objeto"
            )
            continue
    ...
    check(
        target_set > 0,
        "cache contem partidas do Set 18",
    )
    ...
    check(
        ...
        "nenhum campeao nao elegivel foi selecionado como carry",
    )

    check(
        not friendly_name_failures,
        "selecoes possuem display_name amigavel no catalogo",
    )

    check(
        selected_carries + no_carry == transformed,
        "cada partida transformada terminou com carry valido ou ausencia explicita de carry",
    )

    print()
    print("-" * 92)
    print("AMOSTRA REAL - NOMES AMIGAVEIS")
    print("-" * 92)
    ...
    print(f"Falhas de transformacao        : {failed_transform}")
    print(f"Sem PUUID resolvido            : {missing_puuid}")
    print(f"Com carry valido               : {selected_carries}")
    print(f"Sem carry (explicito)          : {no_carry}")
    print(f"Com tank selecionado           : {selected_tanks}")
    print(f"Com support selecionado        : {selected_supports}")
    print(f"Unidades fora do catalogo      : {uncatalogued_units}")
    print(f"Carries nao elegiveis          : {len(selected_noneligible_carries)}")
    print(f"Falhas de display_name         : {len(friendly_name_failures)}")

    if transform_errors:
        print()
        print("Primeiras falhas de transformacao:")
        for error in transform_errors[:10]:
            print(f"  - {error}")

    print()
    print("=" * 92)
    print("RESULTADO: FLUXO REAL V2 VALIDADO ATE O ROLE INFERENCE")
    print("=" * 92)
