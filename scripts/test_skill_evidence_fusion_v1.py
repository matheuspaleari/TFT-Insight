...
def assessment(skill_id, level, score, confidence):
    return FakeAssessment(
        skill=FakeSkill(skill_id=skill_id, title=skill_id),
        score=score,
        level=level,
...
def main():
    ...
    print("=" * 78)
    print("TFT INSIGHT - SKILL EVIDENCE FUSION V1 VALIDATION")
    print("=" * 78)

    passed = 0

    for index, (name, assessments, signals, expected) in enumerate(scenarios, 1):
    ...
    if passed == len(scenarios):
        print("SKILL EVIDENCE FUSION V1: VALIDADA")
        raise SystemExit(0)

    print("SKILL EVIDENCE FUSION V1: AJUSTE NECESSÁRIO")
    raise SystemExit(1)
