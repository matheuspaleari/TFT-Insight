...
# IMPORTANTE:
# database/schema.sql e database/queries.sql são artefatos de código versionáveis.
# Não tratamos o diretório database/ inteiro como runtime.
bad_tracked = sorted(
    item
...
archive_root = ROOT / "scripts/archive"
...
checks.append((
    "Archive legado fora da árvore pública",
    not archive_files,
    (
        f"{len(archive_files)} arquivo(s) encontrado(s) em scripts/archive"
        if archive_files
        else ""
    ),
))
...
# -----------------------------------------------------------------------------
print("=" * 118)
print("#56-V2 / ROADMAP 24.9 - AUDITORIA FINAL PRÉ-COMMIT")
print("=" * 118)
...
if passed != len(checks):
    print()
    print("#56-V2 ROADMAP 24.9: BLOQUEADO — NÃO EXECUTE git add")
    raise SystemExit(1)


print()
print("#56-V2 ROADMAP 24.9: PRONTO PARA STAGE CONTROLADO")
